...
begin
	...
	using Statistics
	using MLDatasets
    # Allow MLDatasets to download in CI
...
md"### Exercise 1.3 – Fashion MNIST"

# ╔═╡ a22aa164-de0d-4e20-85a5-dcec0d12ee43
md"Please only hand in your solution with the checkbox being **unticked**

Run training: $(@bind run_training CheckBox())"
...
Make the dataset compatible with Flux. 
- Figure out the dimensionality of the datapoints 
- Add a singleton colorchannel
- Calculate the one-hot encoded column vectors
- At the end you should have a `x_train`, `y_train`. `x_test` and `y_test`

##### Step 3: Define your data loader
- Set a batchsize (default: 128)
- Use Flux to create your shuffled `train_loader` using your `x_train` and `y_train`

##### Step 4: Define your Loss function and Optimiser
- Define your Optimiser with logit-crossenthrophy (using `setup`)
...
- Compute loss and grandients of the model in respect to the parameters
- Update your optimizer's state (using `update!`)
- Keep track of the losses by logging them

##### Step 6: Visualizing losses
Visualize your logged losses in a simple plot. Use `:log` as scaling for the y-axis.

##### Step 7: Test accuracy
Calculate the accuracy of the trained model using your test data `x_test` and `y_test`

##### Step 8: Predict
Write a function `predict` which predicts the class for a given input.
	
##### Step 9: False positives
implement a function `falsePositiveRate` which calculates the percentage of false positives for a given class when using the model to predict the `dataset_test`.
...
julia_version = "1.8.5"
manifest_format = "2.0"
project_hash = "a47b7952d7a5ff2ca1e947912239af410720a477"
